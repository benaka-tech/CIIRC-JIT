...
class Ui_MainWindow1(object):
    def setupUi(self, MainWindow):
        ...
        self.horizontalLayoutWidget_2.setGeometry(QtCore.QRect(30, 270, 381, 21))
        ...
        self.pushButton_7.setGeometry(QtCore.QRect(160, 300, 131, 23))
        ...
        self.groupBox_5.setGeometry(QtCore.QRect(460, 280, 321, 251))
        ...
        self.label_14.setGeometry(QtCore.QRect(30, 460, 401, 41))
        font = QtGui.QFont()
        font.setPointSize(10)
        ...
        self.horizontalLayoutWidget_4.setGeometry(QtCore.QRect(40, 370, 403, 80))
        ...
        self.pushButton_7.clicked.connect(self.get_temp)
    ...
    def caputure(self):
        ...
        while (self.cap.isOpened()):
            ...
            cv2.imwrite(os.getcwd()+'\\photo-%s.png' % (
                str(datetime.now().strftime("%Y%m%d%H%M%S"))), frame)
            f = open(os.getcwd()+"\\photos\\photos.txt", "a")
            f.write('\n')
            f.write('file:///'+os.getcwd()+'\\photos\\photo-%s.png' % (
                str(datetime.now().strftime("%Y%m%d%H%M%S"))))
            f.close()
            self.label_14.setText("Caputured Photo")
            break
    ...
    def caputure1(self):
        while self.cap1.isOpened():
            ...
            cv2.imwrite(os.getcwd()+'\\ids\\id-%s.png' % (
                str(datetime.now().strftime("%Y%m%d%H%M%S"))), frame)
            f = open(os.getcwd()+"\\ids\\ids.txt", "a")
            f.write('\n')
            f.write('file:///'+os.getcwd()+'\\ids\\id-%s.png' % (
                str(datetime.now().strftime("%Y%m%d%H%M%S"))))
            f.close()
            self.label_14.setText("Caputured Id")
            break
    ...
    def sumbit(self):
        ...
        if name == "" or state == "" or address == "" or emailid == "" or phoneno == "":
            self.label_14.setText("Enter The Data in the Fields")
            return 1
        if not (re.search(regex, emailid)):
            self.label_14.setText("Invalid Email")
            return 1
        if not (re.compile("(0/91)?[7-9][0-9]{9}").match(phoneno)):
            self.label_14.setText("Invalid Phone")
            return 1

        with open(os.getcwd()+"\\photos\\photos.txt", "r") as file:
            for last_line in file:
                pass
        filename = str(last_line)
        with open(os.getcwd()+"\\ids\\ids.txt", "r") as file:
            for last_line1 in file:
                pass
        filename2 = str(last_line1)

        with open(os.getcwd()+"\\temp.txt", "r") as file:
            for last_line2 in file:
                pass
            temp = str(last_line2)
        ...
        if (re.search(regex, emailid) and re.compile("(0/91)?[7-9][0-9]{9}").match(phoneno)):
            ...
            insert_stmt = (
                "INSERT INTO `devotees_details`( `name`, `address`, `state`, `email`, `phone`, `temperature`, `photo`, `id_card`) VALUES (%s, %s, %s, %s, %s, %s ,%s,%s)"

            )
            data = (name, address, state, emailid, phoneno, temp, filename, filename2)
            ...
            conn.close()
            self.label_14.setText("Sumbitted Sucessfully")
            self.lineEdit.clear()
    ...
    def export_report(self):
        ...
        df.to_excel('ds.xls')
        self.label_14.setText("Sucessfully Exported")

    def get_temp(self):
        r = requests.get('http://192.168.4.1:8888/object_temperature/')
        print(r.status_code)
        temp = r.json()
        temp_data = temp['object_temperature']
        if temp_data == "FAIL":
            self.label_14.setText("Try Again")
            return (1)
        self.label_12.setText(temp_data)
        if float(temp_data) >= 36.0 and float(temp_data) <= 38.0:
            self.label_7.setStyleSheet("background-color: green")
        else:
            self.label_16.setStyleSheet("background-color: red")
        f = open("C:\\Users\\Benaka\\PycharmProjects\\pythonProject\\final_copy\\temp.txt", "a")
        f.write('\n')
        f.write(temp_data)
        f.close()
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.pushButton_7.setText(_translate("MainWindow", "Scan Temperature"))
